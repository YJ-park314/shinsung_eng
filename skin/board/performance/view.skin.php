...
preg_match($pattern,stripslashes(str_replace('&','&',$view['content'])), $match);
$img = substr($match['url'],1);
?>
<style>
    .bg{background: url(<?php echo $img ?>) center / cover no-repeat; opacity: .5;}
</style>

<!-- 비쥬얼 영역 시작 -->
<div class="visual view">
    <div class="vis-con wrap">
        <p class="subtitle"><?php echo cut_str(get_text($view['wr_subject']), 70); // 글제목 출력 ?></p>
    </div>
    <div class="bg"></div>
</div>
<!-- 비쥬얼 영역 끝 -->

<div class="ctgr-area view">
    <div class="wrap">
...
<article id="performanceView">
    <h2 class="hidden">본문 영역</h2>
    <section>
        <h3 class="hidden"><?php echo cut_str(get_text($view['wr_subject']), 70); // 글제목 출력 ?></h3>
        <div class="wrap">
            <div class="content">
                <!-- 게시물 상단 버튼 시작 { -->
                <?/*php if($is_admin) { ?>
            ...
            <div class="btn-wrap">
                <a href="<?php echo $list_href ?>" class="btn con bd-blue">목록</a>
                <?php if($is_admin) { ?>
                <?php if ($update_href) { ?><a href="<?php echo $update_href ?>" class="btn con blue">수정</a><?php } ?>
